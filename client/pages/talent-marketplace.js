...
const TalentMarketplace = () => {
    ...
    const [jobData, setJobData] = useState(null); // Stores the job data
    const [loading, setLoading] = useState(true); // To manage loading state

    const { jobId } = router.query;

    const fetchJobData = async () => {
        if (!jobId) return; // Exit if jobId is not available yet
    
        try {
          const response = await axios.get(`http://localhost:4000/api/jobs/${jobId}`); // Fetch the specific job data by ID
      ...
      // Fetch the user role and initialize job fetching
    useEffect(() => {
      const role = localStorage.getItem("role") || "client"; // Get user role from localStorage or set default to client
      setUserRole(role);

      fetchJobData(); // Fetch job details after role is set
    }, [jobId]);
